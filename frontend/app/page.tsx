...
// Add a check to ensure we aren't using localhost in production accidentally
const API_URL = process.env.NEXT_PUBLIC_API_URL || '';

if (!API_URL && process.env.NODE_ENV === 'production') {
  console.warn("⚠️ API_URL is missing in production environment!");
}
...
export default function Home() {
  ...
  console.log('🔍 Debug State:', {
    resumeText: resumeText ? 'HAS CONTENT' : 'EMPTY',
    jobDescription: jobDescription ? 'HAS CONTENT' : 'EMPTY',
    resumeLength: resumeText?.length || 0,
    jobLength: jobDescription?.length || 0,
    loading: loading,
    buttonWillBeDisabled: loading || !resumeText || !jobDescription,
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      console.log('🚀 Making API call to:', `${BACKEND_URL}/api/match-job`);

      const response = await fetch(`${BACKEND_URL}/api/match-job`, {
        method: 'POST',
        body: formData,
      });
